...
anomaly_type = {'concordant','anomaly'};
for i_data =2:2  %choose data
...
if flag_cv_sep %for Mix, seprate regularization for each type of tasks 
    [best_param{1},score] = choose_param_on_valid_sep(train_X,train_Y,valid_X,valid_Y,train_Omega,valid_Omega_c,valid_Omega_b,...
    ...
    [best_param{1},score] = choose_param_on_valid(train_X,train_Y,valid_X,valid_Y,train_Omega,valid_Omega_c,valid_Omega_b,...
...
if ~flag_use_pre_tuned_best_k
    [best_k,score] = decide_k(train_X,train_Y,valid_X,valid_Y,train_Omega,valid_Omega_c,valid_Omega_b,...
        opts_save,prox_param_save,solver,@scorer_without_Z_real_int_complete_cv,500,best_param,j_method,0);   
...
concordant_score = zeros(rep_num,m);
parfor it = 1:rep_num
opts = opts_save;
prox_param = prox_param_save;
[W,Phi,pi] = solver(train_X,train_Y,train_Omega,k,opts,prox_param); %%train the model on training data, W is \Beta, W_softmax is \alpha
W_record(:,:,it) = W(:,:,1);
 
[~,rho] = estimate_fun_without_Z(train_X,train_Y,W,Phi,pi,train_Omega,opts);
[concordant_score(it,:)] = estimate_weights_without_Z_KL(train_X,train_Y,W,Phi,pi,rho,train_Omega,opts); %compute concordant scores
 
[Y_pred_record(:,:,it),Omega_p_record(:,:,it)] = FMR_predict(test_X,test_Y,W,Phi,pi,[],test_Omega_c, test_Omega_b,opts,[]); %use cluster label and \Beta to predict Y

 
[score] = scorer_without_Z_real_int_complete_cv(valid_X,valid_Y,W,Phi,pi,[],valid_Omega_c,valid_Omega_b,opts); %use cluster label and \Beta to predict Y, then compute score
...
concordant_score = concordant_score(idx,:);
...
record_concordant_score_all_groups{i_group} = concordant_score;
end
...
record_concordant_score{j_method} = record_concordant_score_all_groups;
...
disp('##############################')
save(sprintf('result_nMSE_aAUC_MOE_Handling_anomaly_tasks_data_%s_method_%s.mat',data_keyword{i_data},method_name{j_method})); 
...
save(str,'record','record_param','record_best_k','method_set','record_nmi','record_theta','record_Y_pred_all_groups','record_Omega_p_all_groups','record_concordant_score' ,'-v7.3');
end
