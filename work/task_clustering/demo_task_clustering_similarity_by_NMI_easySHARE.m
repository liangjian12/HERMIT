...
method_name = {'Single'}; 
method_set = [1]; %methods to run, 1:Mix, 2:Mix GS
...
record_similarity_all_groups= cell(length(uni),1); %record similarity matrices
...
%solver
solver = @solver_MAR_ADA_without_Z; %Single
...
pi_record = {}; %record pi
pi_each_record = {}; %record pi for each model 
Phi_record = {}; 
...
opts = opts_save;
prox_param = prox_param_save;
[W,Phi,pi,pi_each] = solver(train_X,train_Y,train_Omega,k,opts,prox_param); %%train the model on training data, W is \Beta, W_softmax is \alpha
W_record(:,:,it) = W(:,:,1);
...
pi_each_record{it} = pi_each; %pi for each model 
pi_record{it} = pi; %pi
Phi_record{it} = Phi;
end
similarity_record = zeros(m,m,rep_num);
for it = 1:rep_num
    opts = opts_save;
    prox_param = prox_param_save;
    W = W_record_all{it};
    pi_each = pi_each_record{it};
    Phi = Phi_record{it};
    similarity_record(:,:,it) = estimate_weights_without_Z_multi_method_MAR(train_X,train_Y,W,Phi,[],pi_each,[],train_Omega,opts,'nmi_prob'); 
end
...
record_similarity_all_groups{i_group} = similarity_record;
...
record_similarity{j_method} = record_similarity_all_groups;
...
save(str,'record','record_param','record_best_k','method_set','record_nmi','record_theta','record_Y_pred_all_groups','record_Omega_p_all_groups','record_similarity' ,'-v7.3');
end
