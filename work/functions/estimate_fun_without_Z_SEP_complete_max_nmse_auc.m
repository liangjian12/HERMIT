function [fun_c] = estimate_fun_without_Z_SEP_complete_max_nmse_auc(X,Y,W,Phi,pi,pi_each,Omega_c,Omega_b,opts)
...
%     disp(sprintf('%d / %d',i_type,3))
    ...
    [~,rho] = estimate_fun_without_Z(X,Y(:,idx),W_idx,Phi_idx,pi_each{i_type},Omega_c(:,idx),opts_tmp);
    [fun_c(i_type)] = estimate_fun_giv_rho_without_Z(X,Y(:,idx),W_idx,Phi_idx,pi_each{i_type},rho,Omega_p(:,idx),opts_tmp);
    
    if opts.k > 1
    [nmse_tmp,auc_tmp,nmse_poiss_tmp] = estimate_fun_without_Z_kn_max_nmse_auc(X,Y(:,idx),W_idx,Phi_idx,rho,pi_each{i_type},Omega_p(:,idx),opts_tmp);
    else
    [nmse_tmp,auc_tmp,nmse_poiss_tmp] = estimate_fun_without_Z_k1_nmse_auc(X,Y(:,idx),W_idx,Phi_idx,rho,pi_each{i_type},Omega_p(:,idx),opts_tmp);
    end
