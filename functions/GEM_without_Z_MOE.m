function [rho,W_gate,W,Phi,fun_c] = GEM_without_Z_MOE(X,Y,k,lambda,gamma,prox_param,opts)
...
%     
   ...
   W_gate = sover_softmax_regression(X,rho,opts.lambda_MOE,'L1');
   ...
   param_pre = param;
        ...
        if isfield( opts.prox_param.W, 'capL1_flag')
            if opts.prox_param.W.capL1_flag
                 if iter_out == 1
                    ...
                    prox_param.W.capL1_weight_this = repmat(sum(abs(param.W(:,:,r)),2) < opts.theta * param.pi(r)^(gamma),1,m); 
                 end
            end
        end
    ...
    %E-step
    [fun_c(iter_out),rho] = estimate_fun_without_Z_MOE(X,Y,param.W,param.Phi,W_gate,Omega,opts);
