function [rho,pi,W,Z,Phi,fun_c] = GEM_with_Z_AD_warm_start(X,Y,k,lambda,tau,gamma,prox_param,opts)
...
for iter_out = 1: opts.maxIter_out
   ...
   param_pre = param;
    ...
    for r = 1:k    
        ...
        opts_tmp.fix.Z = opts.fix.Z(:,:,r);
    
        if opts.sep_pen.flag  
            [param.W(:,:,r)] = apg_W_GD_only_W_sep(X,Y,opts_tmp);
        else
            [param.W(:,:,r)] = apg_W_GD_only_W(X,Y,opts_tmp);
        end
    end
