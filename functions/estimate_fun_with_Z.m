function [fun,rho] = estimate_fun_with_Z(X,Y,W,Z,Phi,pi,Omega,opts)
    ...
    if k > 1
        ...
        for r = 1:k    
            [sum_L(:,r),L{r}] = compute_rho_r_with_Z(X,Y,W(:,:,r),Z(:,:,r),Phi(:,:,r),Omega,opts);        
        end
        ...
        n = size(X,1);   
        [sum_L,~] =  compute_rho_r_with_Z(X,Y,W(:,:,1),Z(:,:,1),Phi(:,:,1),Omega,opts);   
        fun = sum(sum_L(:))/n;
        rho = ones(n,1);      
    end
