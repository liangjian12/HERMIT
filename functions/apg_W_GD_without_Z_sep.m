function [W,Phi] = apg_W_GD_without_Z_sep(X,Y,opts)
% Input:
...
for i_type = 1:3
    ...
    if size(opts_tmp.rho,2) > 1
        opts_tmp.rho = opts.rho(:,idx);
    end
    opts_tmp.Y = Y(:,idx);
    ...
    if isfield( opts.prox_param.W, 'capL1_flag')
            if opts.prox_param.W.capL1_flag
                opts_tmp.prox_param.W.capL1_weight_this = opts.prox_param.W.capL1_weight_this(:,idx);
            end
    end
    m_tmp = sum(idx);
...
%         opts_tmp.X_INIT = opts.initial_scale *  randn(dim_x,1);     
    ...
    %APG
%     [x,opts] = apg_plus(@grad_W_GD_without_Z, @soft_thresh_W_GD_without_Z, @opti_close_Phi_without_Z , dim_x, opts_tmp); % x is a vector, containing all the parameters

    [x] = apg(@grad_W_GD_without_Z, @soft_thresh_W_GD_without_Z , dim_x, opts_tmp); % x is a vector, containing all the parameters
