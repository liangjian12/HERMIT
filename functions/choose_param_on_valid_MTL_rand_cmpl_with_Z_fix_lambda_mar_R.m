function [best_param,score] = choose_param_on_valid_MTL_rand_cmpl_with_Z_fix_lambda_mar_R(train_X,train_Y,valid_X,valid_Y,train_Omega,observe_rate_c,valid_Omega_b_save,...
...
valid_Omega_c_save = double(rand(size(valid_Y))<observe_rate_c);
num_useful_task = zeros(n,3); 
...
if parfor_flag
     for i = 1:n
        ...
        z = sum(sum(Z.*Z,3),1);
        useful_task_idx = abs(z) < 1e-6 ;
        num_useful_task(i,:) = [sum(useful_task_idx(opts.task_type == 1)),sum(useful_task_idx(opts.task_type == 2)),sum(useful_task_idx(opts.task_type == 3))];
        
        if sum(useful_task_idx)==0 || sum(~useful_task_idx)==0
            ...
            W(:,~useful_task_idx,:) = repmat(W_k1(:,~useful_task_idx),[1 1 k]);
...
%             score(i,:) = score(i,:) .* [sum(col_idx(opts.task_type == 1)),sum(col_idx(opts.task_type == 2)),sum(col_idx(opts.task_type == 3))];
    ...
    for i = 1:n
        ...
        z = sum(sum(Z.*Z,3),1);
        useful_task_idx = abs(z) < 1e-6 ;
        if sum(useful_task_idx)==0
            score(i,:) = -inf * ones(1,sep_num);
        else
            valid_Omega_c = valid_Omega_c_save;
            valid_Omega_b = valid_Omega_b_save;
  
            valid_Omega_c(:,~useful_task_idx) = 0;
            valid_Omega_b(:,~useful_task_idx) = 0;
             score_find_outlier = scorer(valid_X,valid_Y,W,Phi,pi,[],valid_Omega_c,valid_Omega_b,opts);
            score_all = scorer(valid_X,valid_Y,W_all,Phi_all,pi_all,[],valid_Omega_c,valid_Omega_b,opts);
            opts_tmp = opts; opts_tmp.k=1;
            score_k1 = scorer(valid_X,valid_Y,W_k1,Phi_k1,pi_k1,[],valid_Omega_c,valid_Omega_b,opts_tmp);
