function [x,opts, t] = apg_plus_line_search(grad_f, prox_h, val_f, opti_close , dim_x, opts)
%
...
for k=1:MAX_ITERS
    ...
    if (~FIXED_STEP_SIZE)
        ...
        while cont < 20
            x_new = y - t*g;
            val_f_x_new = val_f(x_new,opts);
            if isnan(val_f_x_new)
                break;
            elseif val_f_x_new <= val_f_y - g_l2_norm * t/2
