function [fun_c] = estimate_auc_without_Z_SEP(X,Y,W,Phi,pi,Omega,opts)
...
%     disp(sprintf('%d / %d',i_type,3))
    ...
    for r = 1:opts.k
        W_idx{r} =  W{r}(:,idx);          
        Phi_idx{r} = Phi{r}(idx,idx);
        opts_tmp.true_W{r} = opts.true_W{r}(:,idx);
    end
     
     
    [fun_c(i_type)] = estimate_fun_without_Z(X,Y(:,idx),W_idx,Phi_idx,pi{i_type},Omega(:,idx),opts_tmp);
