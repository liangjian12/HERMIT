...
load(sprintf('anomaly_idx_task_data_%s.mat',data_keyword{i_data})); %load idx_task, which collects indices representing the type of tasks. 1: concordant tasks, 2: anomaly tasks
...
if ~flag_use_pre_tuned_best_k
    ...
    load(sprintf('tuned_best_k_data_%s_handling_anomaly_tasks.mat',data_keyword{i_data}));
...
str = sprintf('result_FMR_Handling_anomaly_tasks_data_%s.mat',data_keyword{i_data});
save(str,'record','record_param','record_best_k','method_set','record_nmi','record_theta','record_Y_pred_all_groups','record_Omega_p_all_groups' ,'-v7.3');
end
